...
syntax match   juliaKeyword		"\<\%(return\|local\|global\|import\%(all\)\?\|export\|using\|const\)\>"
...
syntax region  juliaRepeatBlock		matchgroup=juliaRepeat start="\<\%(while\|for\)\>" end="\<end\>" transparent
syntax region  juliaBeginBlock		matchgroup=juliaBlKeyword start="\<begin\>" end="\<end\>" transparent
syntax region  juliaFunctionBlock	matchgroup=juliaBlKeyword start="\<function\>" end="\<end\>" transparent
syntax region  juliaMacroBlock		matchgroup=juliaBlKeyword start="\<macro\>" end="\<end\>" transparent
syntax region  juliaQuoteBlock		matchgroup=juliaBlKeyword start="\<quote\>" end="\<end\>" transparent
syntax region  juliaTypeBlock		matchgroup=juliaBlKeyword start="\<type\>" end="\<end\>" transparent
syntax region  juliaImmutableBlock	matchgroup=juliaBlKeyword start="\<immutable\>" end="\<end\>" transparent
syntax region  juliaLetBlock		matchgroup=juliaBlKeyword start="\<let\>" end="\<end\>" transparent
hi Type       ctermfg=lightgreen
hi Type       ctermfg=lightgreen
hi Type       ctermfg=lightgreen
syntax region  juliaDoBlock		matchgroup=juliaBlKeyword start="\<do\>" end="\<end\>" transparent
syntax region  juliaModuleBlock		matchgroup=juliaBlKeyword start="\%(\.\s*\)\@<!\<\%(bare\)\?module\>" end="\<end\>" transparent
...
syntax match   juliaRangeOperator	" in"
...
" edits by mickey
syn region juliaRegion matchgroup=Delimiter start=/(/ matchgroup=Delimiter end=/)/ transparent contains=ALLBUT,juliaErrorItems
syn region juliaRegion matchgroup=Delimiter start=/{/ matchgroup=Delimiter end=/}/ transparent contains=ALLBUT,juliaErrorItems
syn region juliaRegion matchgroup=Delimiter start=/\[/ matchgroup=Delimiter end=/]/ transparent contains=ALLBUT,juliaErrorItems

syn match juliaFunction "\(\(\a\|\.\a\|\._\|\.\.\)[\._\a0-9]*\)\+\ *[(]\@="
...
" redefine colors for strings and comments (this is independent of the color scheme)
...
hi def link juliaRepKeyword	    	Type
hi def link juliaBlKeyword	    	Type
hi def link juliaConditional        Type
hi def link juliaRepeat		    	Type
